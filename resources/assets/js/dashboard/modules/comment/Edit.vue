<template>
    <vue-form :title="$t('form.edit_comment')">
        <template slot="buttons">
            <router-link :to="{ name: 'dashboard.comment' }" class="btn btn-sm btn-secondary" exact>{{ $t('form.back')
                }}
            </router-link>
        </template>
            ...
            <div class="row">
                <form class="col-md-10 offset-md-1" @submit.prevent="edit">
                    <div class="form-group text-center">
                        ...
                        <h6 id="type">{{ (comment.type == 'articles') ? $t('form.articles') : $t('form.discussions')
                            }}</h6>
                    </div>
