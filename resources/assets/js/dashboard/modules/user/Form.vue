    ...
    <div class="row">
        <form class="form col-md-4 offset-md-4" role="form" @submit.prevent="onSubmit">
            <div class="form-group text-center">
                <img :src="user.avatar ? user.avatar : '/images/default.png'" id="avatar" class="rounded-circle"
            ...
            <div class="form-group">
                ...
                <input type="email" class="form-control" id="email" :placeholder="$t('form.email')"
                       v-model="user.email">
            </div>
            <div class="form-group">
                <label for="nickname">{{ $t('form.nickname') }}</label>
                <input type="text" class="form-control" id="nickname" :placeholder="$t('form.nickname')"
                       v-model="user.nickname">
            </div>
            <div class="form-group">
                <label for="website">{{ $t('form.website') }}</label>
                <input type="text" class="form-control" id="website" :placeholder="$t('form.website')"
                       v-model="user.website">
            </div>
            <div class="form-group">
                <label for="description">{{ $t('form.description') }}</label>
                <input type="text" class="form-control" id="description" :placeholder="$t('form.description')"
                       v-model="user.description">
            </div>
            <template v-if="!user.id">
                <div class="form-group">
                    <label for="password">{{ $t('form.password') }}</label>
                    <input type="password" class="form-control" id="password" :placeholder="$t('form.password')"
                           name="password" v-model="user.password">
                </div>
                <div class="form-group">
                    <label for="password_confirmation">{{ $t('form.confirm_password') }}</label>
                    <input type="password" class="form-control" id="password_confirmation"
                           :placeholder="$t('form.confirm_password')" name="password_confirmation"
                           v-model="user.password_confirmation">
                </div>
            </template>
            <div class="form-group">
                <button type="submit" class="btn btn-primary">{{ user.id ? $t('form.edit') : $t('form.create') }}
                </button>
            </div>
        </form>
    </div>
