<template>
    <div class="row">
        <vue-table :title="$t('page.articles')" :fields="fields" api-url="article" :item-actions="itemActions"
                   @table-action="tableActions" show-paginate>
            <template slot="buttons">
                <router-link :to="{ name: 'dashboard.article.create' }" class="btn btn-sm btn-success">{{
                    $t('page.create') }}
                </router-link>
            </template>
        </vue-table>
