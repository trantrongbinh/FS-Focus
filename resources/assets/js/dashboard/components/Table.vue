<template>
    <div :class="wrapperClass" class="ibox">
        ...
        <div class="ibox-content no-padding table-responsive">
                            ...
                            <template v-if="isSpecialField(field.name)">
                                ...
                                <template v-if="field.name == '__component'">
                                    <td :class="field.dataClass" class="component">
                                        <custom-action :api-url="apiUrl" :row-data="item">
                                        </custom-action>
                                    </td>
                                </template>
                            </template>
            ...
            <h3 class="none text-center" v-if="items.length == 0">
                {{ $t('page.nothing') }}
            </h3>
            <table-pagination ref="pagination" v-on:loadPage="loadPage" v-if="showPaginate && items.length> 0">
            </table-pagination>
        </div>
    </div>
</template>
